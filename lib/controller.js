...
const { formatOutput, isSequelizeModel, handleRequestError, execHook, applyMiddleware } = require('./utils');
const { findOne, findAll, createRecord, deleteRecord } = require('./database');
const { methods, errors, hooks } = require('./constants');
	...
	if (options.handlers.put === true) {
		router.put('/:id', (req, res) => {
			...
			model.findOne(buildWhere(methods.PUT, req, options)).then((result) => {
				...
				execHook(hooks.BEFORE_UPDATE, options, model, result);

				return result.save();
			}).then((results) => {
				execHook(hooks.AFTER_UPDATE, options, model, results);
